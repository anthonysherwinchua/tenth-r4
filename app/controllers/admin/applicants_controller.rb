class Admin::ApplicantsController < Admin::BaseController
  ...
  def new
    @form = ApplicantWizardForm::Steps::PersonalInfoStep.new(applicant: Applicant.last)
  end

  def create
    applicant = Applicant.where(id: params[:applicant_wizard_form_steps_personal_info_step][:applicant_attributes][:id]).first_or_initialize
